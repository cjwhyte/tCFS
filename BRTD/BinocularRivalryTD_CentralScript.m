...
% - nan entries correspond to trials where probes caused switches and are therefore excluded
...
for ii = 1:length(hits_dominant)
    errorbar(1:6,hits_dominant(:,ii),hits_dominant_ste(:,ii),'-o', 'linewidth',2, 'Color',[1-ii/length(hits_dominant) 1-ii/length(hits_dominant) 1-ii/length(hits_dominant)])
end 
ax = gca;
set(gca, 'FontName', 'Times')
set(gcf,'Color','w');
...
f = figure(5); hold on
for ii = 1:length(hits_dominant)
    errorbar(1:6,hits_suppressed(:,ii),hits_suppressed_ste(:,ii),'-o', 'linewidth',2, 'Color',[1-ii/length(hits_dominant) 1-ii/length(hits_dominant) 1-ii/length(hits_dominant)])
end
ax = gca;
set(gca, 'FontName', 'Times')
set(gcf,'Color','w');
